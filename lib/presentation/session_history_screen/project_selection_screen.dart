    ...
    try {
      ...
      for (final s in sessionsRaw) {
        ...
        if (startDt != null) {
          ...
          // For Mahindra EV PoC, skip adding raw costs for March, April, May 2026 (overridden)
          final isHistoricalEV = key == 'mahindra ev poc' &&
              startDt.year == 2026 &&
      ...
      // Add historical overrides (Track = 1,263,500, Accessories = 215,219, Workshop = 245,000) to Mahindra EV PoC
      final evCard = cardMap['mahindra ev poc'];
      if (evCard != null) {
        const historicalTrack = 1263500.0;
        const historicalAccessories = 215219.0;
        const historicalWorkshop = 245000.0;
        evCard.subtotalExcl += (historicalTrack + historicalAccessories + historicalWorkshop);
        evCard.totalInclGst += (historicalTrack + historicalAccessories + historicalWorkshop) * 1.18;
      }
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          ...
          // Floating vehicle image – overlaps bottom border
